# Andrej Tetkić - Portfolio

A minimalist, black‑themed portfolio showcasing my work as a software developer and AI/ML practitioner from Novi Sad, Serbia. I build practical tools and systems people enjoy using - from Blender add‑ons and IoT devices to AI‑powered software.

## About me
- 4th‑year student and active freelance developer
- Focused on Python, AI/ML, Blender add‑ons, and IoT systems
- I explore new uses of AI and run my own personal research projects
- Honorable mention at the International AI Olympiad (missed bronze by 0.3/600)

## Highlights
- 20+ clients; tools used by 2600+ users
- Python, JavaScript, Flutter; Embedded/IoT and computer vision experience
- Awards: Galaksija Kup (2nd), National AI Olympiad (1st), International AI Olympiad (honorable mention)

## Projects (selection)
- Parking Tracker - smart parking occupancy device and system
- Drag & Drop Import - Blender add‑on for streamlined asset importing
- LEGO Dataset - ML dataset and CV experiments

## Links
- Website: https://andrejtetkic.github.io (https://tetkic.com)
- GitHub: https://github.com/andrejtetkic
- Fiverr: https://www.fiverr.com/andrejtetkic
- Gumroad: https://gumroad.com/andrejtetkic
- BlenderMarket: https://blendermarket.com/creators/andrejtetkic

## Contact
- Email: andrej@tetkic.com

---
This repository hosts the source for my portfolio website. Feel free to open issues for typos or broken links.
